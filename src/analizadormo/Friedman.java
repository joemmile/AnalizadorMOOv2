...
public class Friedman {
    ...
    public static void FriedmanTest_MAX(String Indicador) throws FileNotFoundException, IOException
    {
    FileOutputStream fos = new FileOutputStream(""+Indicador+"Friedman.tex", false);
    ...
    for(int x=0;x<AnalizadorMO.nombres_problemas.size();x++){
    for(int w=0;w<AnalizadorMO.iteraciones;w++)
    {
        //EN TOTAL HAY ITERACIONES X PROBLEMA DE FILAS!!!!
        datasets.add(w);//datasets son las filas segun entiendo
    }}
    
    for(int w=0;w<AnalizadorMO.algoritmos;w++)
    {
        algoritmos.add(AnalizadorMO.nombres_algoritmos.get(w));//en algoritmos van los nombres de las columnas pues
        datos.add(new Vector());//y se guarda un vector columna por algoritmo en datos
    }
    for(i=0;i<AnalizadorMO.algoritmos;i++)
    {
    for(int w=0;w<AnalizadorMO.nombres_problemas.size();w++){
    String problema=AnalizadorMO.nombres_problemas.get(w);
        FileReader fr=new FileReader("data/"+problema+"/"+problema+"_"+Indicador+"_Alg"+i);
        BufferedReader bf=new BufferedReader(fr);
        for(j=0;j<AnalizadorMO.iteraciones;j++)
        {
            ((Vector)datos.elementAt(i)).add(Double.parseDouble(bf.readLine()));//agrega un valor puest 
        }
    ...
    for (i=0; i<datasets.size(); i++) {
      for (j=0; j<algoritmos.size(); j++) {
	    ...
	    orden = new ParejaMAX[datasets.size()][algoritmos.size()];
	    for (i=0; i<datasets.size(); i++) {
	    	for (j=0; j<algoritmos.size(); j++){
	    		orden[i][j] = new ParejaMAX (j,mean[i][j]);
	    	}
	    	Arrays.sort(orden[i]);
	    }

	    /*building of the rankings table per algorithms and data sets*/
	    rank = new ParejaMAX[datasets.size()][algoritmos.size()];
	    posicion = 0;
	    for (i=0; i<datasets.size(); i++) {
	    	for (j=0; j<algoritmos.size(); j++){
	    		encontrado = false;
	    		...
	    		rank[i][j] = new ParejaMAX(posicion,orden[i][posicion-1].valor);
	    	}
	    }
	    ...
	    ps.println("\\documentclass[a4paper,10pt]{article}\n" +
	    		"\\usepackage{graphicx}\n" +
	    		"\\usepackage{lscape}\n" +
	    		"\\title{Results}\n" +
	    		"\\author{}\n" +
            ...
            ps.println("\\begin{table}[!htp]\n" +
            "\\centering\n" +
            "\\caption{Average Rankings of the algorithms\n}" +
            "\\begin{tabular}{c|c}\n" +
            "Algorithm&Ranking\\\\\n\\hline");
            for (i=0; i<algoritmos.size();i++) {
              ps.println((String)algoritmos.elementAt(i)+"&"+Rj[i]+"\\\\");
            }
        ...
        ps.print("P-value computed by Friedman Test: " + pFriedman +".\\newline\n\n");

	    /*Compute the Iman-Davenport statistic*/
	    iman = ((datasets.size()-1)*friedman)/(datasets.size()*(algoritmos.size()-1) - friedman);
	    ps.println("Iman and Davenport statistic considering reduction performance (distributed according to F-distribution with "+(algoritmos.size()-1)+" and "+ (algoritmos.size()-1)*(datasets.size()-1) +" degrees of freedom: "+iman+".\n\n");

		pIman = FishF(iman, (algoritmos.size()-1),(algoritmos.size()-1) * (datasets.size() - 1));
		ps.print("P-value computed by Iman and Daveport Test: " + pIman +".\\newline\n\n");
	    ...
	    for (i=0; i<algoritmos.size();i++) {
	    	...
	    	if (i==0) {
                ...
                ps.println((algoritmos.size()-i) + "&" + algoritmos.elementAt(pos) + "&" +
        ...
        ps.println("Bonferroni-Dunn's procedure rejects those hypotheses that have a p-value $\\le"+0.05/(double)(algoritmos.size()-1)+"$.\n\n");
	    
	    parar = false;
	    for (i=0; i<algoritmos.size()-1 && !parar; i++) {
	    	if (Pi[i] > ALPHAiHolm[i]) {	    		
	    		ps.println("Holm's procedure rejects those hypotheses that have a p-value $\\le"+ALPHAiHolm[i]+"$.\n\n");
	    		parar = true;
	    	}
	    }
	    ...
	    for (i=algoritmos.size()-2; i>=0 && !parar; i--) {
	    	if (Pi[i] <= ALPHAiHolm[i]) {	    		
	    		ps.println("Hochberg's procedure rejects those hypotheses that have a p-value $\\le"+ALPHAiHolm[i]+"$.\n\n");
	    		parar = true;
	    	}
	    }
	    ...
	    if (otro == true) {
	    	ps.println("Hommel's procedure rejects all hypotheses.\n\n");
	    } else {
	    	j++;
	    	ps.println("Hommel's procedure rejects those hypotheses that have a p-value $\\le"+0.05/(double)j+"$.\n\n");
	    }
        ...
        ps.println("Bonferroni-Dunn's procedure rejects those hypotheses that have a p-value $\\le"+0.1/(double)(algoritmos.size()-1)+"$.\n\n");
	    ...
	    for (i=0; i<Pi.length; i++) {	    	
            ps.println((i+1) + "&" + ordenAlgoritmos[i] + "&" + Pi[i] +
            		"&" + adjustedP[i][0] +
               		"&" + adjustedP[i][1] +
               		"&" + adjustedP[i][2] +
               		"&" + adjustedP[i][3] + "\\\\");
	    ...
	    for (i=0; i<order.length; i++) {
            ps.println((order.length-i) + "&" + ordenAlgoritmos[order[i]] + "&" +
        ...
        ps.println("Nemenyi's procedure rejects those hypotheses that have a p-value $\\le"+0.05/(double)(order.length)+"$.\n\n");
	    ...
	    for (i=0; i<order.length && !parar; i++) {
	    	if (Pi[i] <= ALPHAiShaffer[i]) {	    		
	    		ps.println("Shaffer's procedure rejects those hypotheses that have a p-value $\\le"+ALPHAiShaffer[i]+"$.\n\n");
	    		parar = true;
	    	}
	    }
	    ...
	    if (algoritmos.size() < 9) {
			...
			ps.println("Bergmann's procedure rejects these hypotheses:\n\n");
			ps.println("\\begin{itemize}\n\n");
			for (i=0; i<cuadro.length;i++) {
				for (j=i+1; j<cuadro.length;j++) {					
					if (cuadro[i][j] == false) {
						ps.println("\\item "+algoritmos.elementAt(i)+" vs. "+algoritmos.elementAt(j));
					}
				}
			}
        ...
        ps.println("Nemenyi's procedure rejects those hypotheses that have a p-value $\\le"+0.1/(double)(order.length)+"$.\n\n");
	    ...
	    for (i=0; i<Pi.length; i++) {	    	
            ps.println((i+1) + "&" + algoritmos.elementAt(parejitas[order[i]].i) + " vs ." + algoritmos.elementAt(parejitas[order[i]].j) + "&" + Pi[i] +
            		"&" + adjustedP[i][0] +
               		"&" + adjustedP[i][1] +
               		"&" + adjustedP[i][2] +
               		"&" + adjustedP[i][3] + "\\\\");
	    ...
	    ps.println("\\end{landscape}\\end{document}");
	}
    
    public static void FriedmanTest_MIN(String Indicador) throws FileNotFoundException, IOException
    {
    FileOutputStream fos = new FileOutputStream(""+Indicador+"Friedman.tex", false);
    PrintStream ps=new PrintStream(fos);
    Vector algoritmos;
    ...
    String problema=AnalizadorMO.nombres_problemas.get(w);
        ...
        for(j=0;j<AnalizadorMO.iteraciones;j++)
        {
            ...
            System.out.println("data/"+problema+"/"+problema+"_"+Indicador+"_Alg"+i);
            System.out.println(bf);
            System.out.println(j);
            String micadenita = bf.readLine();
            System.out.println(micadenita);
            ((Vector)datos.elementAt(i)).add(Double.parseDouble(micadenita));//agrega un valor puest 
        }
    ...
    for (i=0; i<datasets.size(); i++) {
      for (j=0; j<algoritmos.size(); j++) {
	    ...
	    /*We use the pareja structure to compute and order rankings*/
	    orden = new ParejaMIN[datasets.size()][algoritmos.size()];
	    for (i=0; i<datasets.size(); i++) {
	    	for (j=0; j<algoritmos.size(); j++){
	    		orden[i][j] = new ParejaMIN (j,mean[i][j]);
	    	}
	    	Arrays.sort(orden[i]);
	    }

	    /*building of the rankings table per algorithms and data sets*/
	    rank = new ParejaMIN[datasets.size()][algoritmos.size()];
	    posicion = 0;
	    for (i=0; i<datasets.size(); i++) {
	    	for (j=0; j<algoritmos.size(); j++){
	    		encontrado = false;
	    		...
	    		rank[i][j] = new ParejaMIN(posicion,orden[i][posicion-1].valor);
	    	}
	    }

...
public class HypervolumenWFG {
        ...
        for (int x = 0; x < frente_aproximado.size(); x++) {
            for (int j = 0; j < AnalizadorMO.objetivos; j++) {
                ...
                if (frente_aproximado.get(x)[j] <= 1.0 && frente_aproximado.get(x)[j] >= 0.0) {
                    ...
                    front[x][j] = 0.0;
                } else if (frente_aproximado.get(x)[j] < 0.0) {
                    front[x][j] = 1.0;
                }
            }
        }
    ...
    static int dominates2way(POINT p, POINT q, int k) // returns -1 if p dominates q, 1 if q dominates p, 2 if p == q, 0 otherwise 
    ...
    static boolean dominates1way(POINT p, POINT q, int k) // returns true if p dominates q or p == q, false otherwise 
    ...
    static void makeDominatedBit(FRONT ps, int p) // creates the front ps[0 .. p-1] in fs[fr], with each point bounded by ps[p] and dominated points removed 
    {
        int l = 0;
        int u = p - 1;
        for (int i = p - 1; i >= 0; i--) {
    ...
    static double hv2(FRONT ps, int k) // returns the hypervolume of ps[0 .. k-1] in 2D 
    ...
    static double inclhv(POINT p) // returns the inclusive hypervolume of p
    {
        double volume = 1;
        for (int i = 0; i < n; i++) {
    ...
    static double inclhv2(POINT p, POINT q) // returns the hypervolume of {p, q}
    {
        double vp = 1;
        double vq = 1;
        double vpq = 1;
    ...
    static double inclhv3(POINT p, POINT q, POINT r) // returns the hypervolume of {p, q, r}
    {
        double vp = 1;
        double vq = 1;
        double vr = 1;
    ...
    static double inclhv4(POINT p, POINT q, POINT r, POINT s) // returns the hypervolume of {p, q, r, s}
    {
        double vp = 1;
        double vq = 1;
        double vr = 1;
    ...
    static double exclhv(FRONT ps, int p) // returns the exclusive hypervolume of ps[p] relative to ps[0 .. p-1] 
    {
    ...
    static double hv(FRONT ps) // returns the hypervolume of ps[0 ..] 
    {
